...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let mintThirdBlock = chain.mineBlock([
            ...
            Tx.contractCall("level-iii", "mint-level-III", [types.uint(4), types.uint(5), types.uint(6)], wallet_1.address),
        ]);

        let mintBlock2 = chain.mineBlock([
            Tx.contractCall("level-iii", "get-token-uri ", ['u0'], wallet_1.address),
            Tx.contractCall("level-iii", "get-token-uri ", ['u1'], wallet_1.address),
        ]);
